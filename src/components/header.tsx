        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between"> 
          ...
          <div className="hidden md:flex items-center gap-7">
            {navItems.map(({ label, href }) => {
              const isActive =
                (label === "Home" && pathname === "/") ||
                (label === "Product" && pathname && pathname.startsWith("/product")) ||
                (label === "About" && pathname && pathname.startsWith("/about")) ||
                (label === "Team" && pathname && pathname.startsWith("/team"));
              return (
                <Link
                  key={label}
                  href={href}
                  className={`px-3 py-2 rounded-xl text-base md:text-lg transition-colors focus:outline-none focus:ring-2 focus:ring-white/30 ${
                    isActive
